...
def getMoravecKps(img_path, win_size=3, win_offset=1, nonMax_size=5, nonMaxValue=0, nFeature=-1, thCRF=-1):
    ...
    cv2.imwrite("./pictures/keymap.jpg", keymap)
    keymap = np.where(keymap < mean_c, 0, keymap)
    cv2.imwrite("./pictures/keymap_th.jpg", keymap)
    ...
    cv2.imwrite("./pictures/keymap_nonMax.jpg", keymap)
    ...
    return img_o, kps, img_kps

if __name__ == '__main__':
    img_oo, kps, img1 = getMoravecKps('./pictures/laozi.jpg', nFeature=300)

    plt.figure(figsize=(10, 14))

    plt.subplot(221)
    plt.imshow(img_oo, cmap='gray')
    plt.title('Origin')
